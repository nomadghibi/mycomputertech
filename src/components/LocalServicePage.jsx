...
const LocalServicePage = ({ page }) => {
  ...
  const serviceSchema = {
    ...
    areaServed: [{ '@type': 'City', name: page.city || page.eyebrow }],
  ...
  return (
    <>
      ...
      <section className="relative isolate overflow-hidden bg-slate-950 py-12 text-white sm:py-20">
        ...
        <div className="mx-auto grid max-w-7xl gap-10 px-4 sm:px-6 lg:grid-cols-[1.05fr_0.95fr] lg:items-center lg:px-8">
          ...
          <div className="rounded-lg border border-white/15 bg-white p-6 text-slate-900 shadow-2xl">
            <p className="text-sm font-bold uppercase tracking-[0.18em] text-cyan-700">Local Service Snapshot</p>
            <h2 className="mt-3 text-xl font-bold text-slate-950 sm:text-2xl">Clear repair help for {page.eyebrow}</h2>
            <div className="mt-5 grid gap-3">
              {page.heroCards.map((card) => (
                <div key={card.label} className="rounded-md bg-slate-50 px-4 py-3">
        ...
        <div className="mx-auto grid max-w-7xl gap-8 px-4 sm:px-6 lg:grid-cols-[0.95fr_1.05fr] lg:px-8">
          <div>
            ...
            <h2 className="mt-3 text-2xl font-bold tracking-tight text-slate-950 sm:text-4xl">
              Practical local support for the problem in front of you
            </h2>
      ...
      <section className="bg-slate-50 py-14 sm:py-16">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-3xl text-center">
            ...
            <h2 className="mt-3 text-2xl font-bold tracking-tight text-slate-950 sm:text-4xl">
              Common problems this service page should rank for
            </h2>
          </div>
          <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {page.services.map((service) => (
              <article key={service} className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
                <FaCheckCircle className="h-5 w-5 text-cyan-700" />
                <p className="mt-4 text-sm leading-6 text-slate-700">{service}</p>
              </article>
            ))}
          </div>
        </div>
        ...
        <div className="mx-auto grid max-w-7xl gap-8 px-4 sm:px-6 lg:grid-cols-[1fr_auto] lg:items-center lg:px-8">
          <div>
            ...
            <p className="mt-4 max-w-3xl text-blue-50">
              If the device is slow, unstable, or not working the way it should, call My Computer Tech
      ...
      <section className="bg-white py-12">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="rounded-lg border border-slate-200 bg-slate-50 p-6">
            <p className="text-sm font-semibold uppercase tracking-[0.2em] text-blue-700">Related Pages</p>
            <div className="mt-4 flex flex-wrap gap-3">
              <Link to="/services" className="rounded-md bg-blue-700 px-4 py-2 text-sm font-semibold text-white transition hover:bg-blue-800">
                All Services
              </Link>
              <Link to="/service-areas" className="rounded-md border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:border-blue-300 hover:text-blue-800">
                Service Areas
              </Link>
